...
{
    public static function archiveSizes($post, $column_count)
    {
        if (has_term('magazines', 'sd-product-cat', $post->ID)) {
            return sprintf(
                ...
                round(100 / $column_count / SD_PRODUCT_IMAGE_FACTOR),
                round(PostsPreview::BOOTSTRAP_COLUMNS['xl'] / $column_count / SD_PRODUCT_IMAGE_FACTOR)
            );
        } else {
            return PostsPreview::sizes($column_count);
        }
    }
}
